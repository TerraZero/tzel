...
module.exports = class FileManager {
  ...
  inject(helper, boot) {
    ...
    this._mods = boot.mods();
    this._root = boot.root();
    this._settings = boot.settings().files;
    this._settings.storages['root'] = boot.root();
  }
  ...
  isFile(file) {
    return file instanceof File.class;
  }
  ...
  getStorageRoot(storage) {
    ...
    if (path.isAbsolute(root)) {
      ...
      return path.join(this._root, root);
    }
  }
  ...
  listFiles(file) {
    const files = [];
    const listed = this.list(file);

    for (const index in listed) {
      files.push(new File(file.path(), listed[index]));
    }
    return files;
  }
