...
module.exports = class Boot {
  ...
  registerClassesExtender(extender, data) {
    ...
    if (data.extended.use !== null) {
      data.extended.file = use(data.extended.use).__file;
      this.registerClassesExtender(extender, this._classes[data.extended.file]);

      for (const index in this._providers) {
        this._providers[index].extend(data, this._classes[data.extended.file]);
      }
    }
  }
