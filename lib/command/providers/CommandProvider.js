...
module.exports = class CommandProvider extends Provider {
  ...
  invoke(string, data) {
    ...
    subject = new subject(string, data.get('names'), data.get('annotation'));

    if (data.get('annotation').type === 'generate') {
      Injector.injectingArray(subject, ['helper.norm', 'replacer.command', 'helper.options'], 'injectGenerate');
    }
